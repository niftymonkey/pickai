...
- A change the web app renders: a rendered check on the dev server, driven with `playwright-cli` and never the Playwright MCP, with the screenshot actually read. The dev server is the default judge (Mark, 2026-09-02); a production build is made only with a reason, such as a performance-judged check or a build-time concern, and lint, types, and tests run as their own separate checks. Actor: the agent.
