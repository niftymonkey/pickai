...
The library is the root package: source in `src/`, built with tsup. `docs/` is the Astro documentation site. `web/` is the Next.js decision-surface prototype, kept as a visual reference and never reused; see the prototype boundary in `design/agents/feature-playbook.md`. All three are pnpm workspace packages.
