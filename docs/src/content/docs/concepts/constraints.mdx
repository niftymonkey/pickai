...
Constraints don't change scores. They shape the final selection by controlling which scored models make it into [`recommend()`](/reference/recommend/) results. pickai ships three built-in constraints: [`perProvider()`](/reference/per-provider/), [`perFamily()`](/reference/per-family/), and [`perModel()`](/reference/per-model/).
...
Limits how many entries of the same physical model appear in results, using model identity ([`matchesModel()`](/reference/matches-model/)) rather than the coarse `family` tag. The catalog lists popular models once per provider and reseller; without this constraint, a top-10 can be ten listings of one model.

```ts
import { perModel } from "pickai";

// One slot per physical model, regardless of how many providers list it
